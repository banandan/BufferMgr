...
public class BufferHashTable {
    ...
    public void addPageNumber(PageId page_number,int frame_number){
		...
		BufferHTEntry newEntry = new BufferHTEntry(page_number.pid,frame_number);
    ...
    /**
     * This methods removes the tuple (page number, frame number) from the hash table
     * @param page_number
     */
    ...
    public void printHashTable(){
		for(int i=0; i < HTSIZE; i++){
			...
			while(entry != null){
				System.out.println(entry.getPageNumber() + " " + entry.getFrameNumber());
				entry = entry.getNext();
			}
		}
	}
