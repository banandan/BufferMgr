...
/**
 * 
 * This class  provides the interface for a client (a higher level program that calls the Buffer Manager) 
 * to allocate/de-allocate pages on disk, to bring a disk page into the buffer pool 
 * and pin it, and to unpin a page in the buffer pool.
 * @author Balamurugan Anandan
 *
 */
public class BufMgr implements GlobalConst{
	private enum Policy {CLOCK,MRU,LRU,LH};
	 ...
	 */
	public BufMgr(int numbufs, String replacerArg) throws ReplacerException, InvalidFrameSizeException {
   ...
   */
  public void pinPage(PageId pin_pgid, Page page, boolean emptyPage) throws BufferPoolExceededException, Exception {
     int frameLocation = hashTbl.getFrameNumber(pin_pgid);
     if( frameLocation == -1){
    	 int candidateFrame = replacement.findReplacement();
    	 if (candidateFrame == -1){
    		 throw new BufferPoolExceededException(null, "BUFFER_POOL_EXCEEDED");
    	 }
 ...
 * @throws DiskMgrException 
   */
  public PageId newPage(Page firstpage, int howmany) throws PageAllocationException, DiskMgrException { 
	  ...
	  try{
		  ...
		  try{
			  ...
			  throw new DiskMgrException(e1,"DB.java: deallocate_page() failed");
		  }
		  tpageid = null;
	  }
 ...
 * @throws BufMgrException 
   */
  public void freePage(PageId globalPageId) throws PagePinnedException, DiskMgrException, BufMgrException { 
	  ...
	  if(frameLocation != -1 && bufDescr[frameLocation].getPinCount() > 1){
		  throw new PagePinnedException(null,"BUFMGR: CANNOT DEALLOCATE A PINNED PAGE");
	  }
	  if(!entered){
	  Page tpage = new Page();
	  try{
